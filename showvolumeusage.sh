...
# === Longhorn Volume Usage Reporter v5.1 (totals fixed) ===
WATCH_INTERVAL=0
NAMESPACE=""

RED="\e[31m"; YELLOW="\e[33m"; GREEN="\e[32m"; CYAN="\e[36m"; RESET="\e[0m"; BOLD="\e[1m"
...
print_report() {
  ...
  # ⬇️ استفاده از process substitution تا حلقه در subshell اجرا نشود
  while IFS=$'\t' read -r volname size actual node; do
    ...
    if [[ "$size" -gt 0 ]]; then usage_pct=$((100 * actual / size)); else usage_pct=0; fi

    if (( usage_pct >= 90 )); then color=$RED
    elif (( usage_pct >= 70 )); then color=$YELLOW
    else color=$GREEN; fi
  ...
  done < <(kubectl -n longhorn-system get volumes.longhorn.io -o json \
           | jq -r '.items[] | [.metadata.name, .spec.size, .status.actualSize, .status.ownerID] | @tsv')

  # گارد تقسیم‌برصفر برای درصد کل
  if [[ "$total_size" -gt 0 ]]; then
    total_pct=$((100 * total_used / total_size))
  else
    total_pct=0
  fi

  total_size_h=$(numfmt --to=iec --suffix=B "$total_size" 2>/dev/null)
  total_used_h=$(numfmt --to=iec --suffix=B "$total_used" 2>/dev/null)

  echo "------------------------------------------------------------------------------------------------------------------------------------------------------"
  echo -e "${BOLD}TOTAL${RESET}"
  printf "%-60s %-10s %-10s ${CYAN}%-10s${RESET}\n" " " "$total_size_h" "$total_used_h" "${total_pct}%"
}
